sap.ui.define([
	...
	return BaseController.extend("nus.edu.sg.ot_ea_detailedreport.controller.report", {
		formatter: Formatter,
		onInit: function () {
		...
		handleValueHelpRequestId: function (oEvent) {
			...
			aFilter.push(new sap.ui.model.Filter("CLAIM_REQUEST_TYPE", sap.ui.model.FilterOperator.EQ, '103'));
		...
		openClaimNoValueHelpPopUp: function () {
			...
			if (!this._oDialogAddRequestId) {
				this._oDialogAddRequestId = Fragment.load({
					id: oView.getId(),
					name: "nus.edu.sg.ot_ea_detailedreport.fragment.RequestIdValueHelpDialog",
					controller: this
						...
						if (NoSearchHelpPopUp !== 'Y') {
							...
							if (!that._oDialogAddUlu) {
								that._oDialogAddUlu = Fragment.load({
									id: oView.getId(),
									name: "nus.edu.sg.ot_ea_detailedreport.fragment.UluValueHelpDialog",
									controller: that
								}).then(function (oDialog) {
									oView.addDependent(oDialog);
									return oDialog;
			...
			if (NoSearchHelpPopUp !== 'Y') {
				...
				if (!this._oDialogAddFdlu) {
					this._oDialogAddFdlu = Fragment.load({
						id: oView.getId(),
						name: "nus.edu.sg.ot_ea_detailedreport.fragment.FdluValueHelpDialog",
						controller: this
					}).then(function (oDialog) {
						oView.addDependent(oDialog);
						return oDialog;
				...
				oDataModel.read("/CHRS_JOB_INFO", {
					...
					success: function (oData) {
						if (oData) {
							...
							// 		id: oView.getId(),
							// 		name: "nus.edu.sg.ot_ea_detailedreport.fragment.StaffValueHelpDialog",
							// 		controller: that
							// 	}).then(function (oDialog) {
							// 		oView.addDependent(oDialog);
							// 		return oDialog;
						...
						if (oData.results.length) {
							...
							if (!this._oDialogVerifer) {
								this._oDialogVerifer = Fragment.load({
									id: oView.getId(),
									name: "nus.edu.sg.ot_ea_detailedreport.fragment.VerifierValueHelpDialog",
									controller: this
								}).then(function (oDialog) {
									oView.addDependent(oDialog);
									return oDialog;
						...
						if (oData.results.length) {
							...
							if (!this._oDialogApprover) {
								this._oDialogApprover = Fragment.load({
									id: oView.getId(),
									name: "nus.edu.sg.ot_ea_detailedreport.fragment.ApproverValueHelpDialog",
									controller: this
								}).then(function (oDialog) {
									oView.addDependent(oDialog);
									return oDialog;
		...
		handleValueHelpApproverOT: function (oEvent) {
			var oView = this.getView();
			var EclaimSrvModel = this.getComponentModel("Eclaims");
			var aFilter = [];
			var orFilter = [];
			var andFilter = [];
			var userRoleGrp = this.AppModel.getProperty("/userRoleGrp"); //Super Admin
			...
			//					
			/*if (userRoleGrp === "NUS_CHRS_ECLAIMS_SUPER_ADMIN" || (!!claimAuthorizations && claimAuthorizations.length > 0)) {
				EclaimSrvModel.read("/v_ot_approver_lists", {
					filters: aFilter,
					success: function (oData) {
						if (oData.results.length) {
							this.AppModel.setProperty("/Approver", oData.results);
		...
		handleSearchApprover: function (oEvent) {
			...
			oDataModel.read("/v_ot_approver_lists", {
				filters: [filtersGrp],
				// urlParameters: {
				// 	"$select": "STF_NUMBER,FULL_NM"
				// },
		...
		searchFilter: function (oEvent) {
			...
			if (this.AppModel.getProperty("/claimRequest/UluList").length > 0 || userRoleGrp === "NUS_CHRS_ECLAIMS_SUPER_ADMIN") {
				...
				if (!!selectedItemsUlu && selectedItemsUlu.length > 0) {
					...
					andFilter.push(new sap.ui.model.Filter("PROCESS_CODE", FilterOperator.EQ, "103"));
		...
		_fnHandleDataToExport: function (aData) {
			...
			aData = aData.map(function (item) {
				item.OPT_FOR_COMP = item.IS_PH === 5 ? "X" : "";
				if (item.IS_PH === 2 || item.IS_PH === 5) {
					item.IS_PH = "X";
				} else {
					item.IS_PH = "";
				}
			...
			oSettings = {
				...
				dataSource: aData,
				fileName: "Overtime(EA) Detailed Report.xlsx"
			};

			oSheet = new Spreadsheet(oSettings);
			oSheet.build()
				.then(function () {
					MessageToast.show('Overtime(EA) Detailed Report exported successfully..!!');
				})
				.finally(function () {
					oSheet.destroy();
				});
